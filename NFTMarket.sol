...
library UintLibrary {
    ...
    function bp(uint value, uint bpValue) internal pure returns (uint) {
        return value.mul(bpValue).div(100000);
    }
}
...
contract ExchangeV1 is Ownable, ExchangeDomainV1 {
    ...
    //for ether deposit
    event Deposit(uint tokenId, uint wad);
    event Withdraw(uint tokenId,uint wad);
    event Approval(address indexed src, address indexed guy, uint wad);
    //for nft  Deposit
    //event nftDeposit(address indexed src,address indexed owner,uint nftId);
    event DoChange(address indexed seller,address indexed buyer,uint selling,uint buying);
    event Buy(
    ...
    function exchange(
        ...
        //require(balanceOf[order.key.owner]>=amount);
        ...
        */
        ...
        transferWithFeesPossibility(order.key.sellAsset, amount, order.key.owner, buyer, false, buyerFee, order.sellerFee, order.key.buyAsset);
        transferWithFeesPossibility(order.key.buyAsset, paying, msg.sender, order.key.owner, false, order.sellerFee, buyerFee, order.key.sellAsset);
        //balanceOf[order.key.owner]-=amount;
    ...
    function prepareMessage(Order memory order) public pure returns (string memory) {
        //require(balanceOf[order.key.owner]>=order.selling);
    ...
    function recover(bytes memory sig) public pure returns (uint8,bytes32,bytes32) {
      bytes32 r;
      bytes32 s;
      uint8 v;
    ...
    function deposit(uint tokenId)external payable{
        ...
        emit Deposit(tokenId,_amount);
    }
    function transferOrder(uint tokenId,uint newTokenId)public returns(bool){
        uint balanceToTransfer=balanceOf[tokenId][msg.sender];
        balanceOf[newTokenId][msg.sender]=balanceToTransfer;
        delete balanceOf[tokenId][msg.sender];
    }
    function withdraw(uint tokenId,address payable to)payable external{
        uint withdrawBalance=balanceOf[tokenId][msg.sender];
        to.transfer(withdrawBalance);
        delete balanceOf[tokenId][msg.sender];
        emit Withdraw(tokenId,withdrawBalance);
    }
    /*
    require(allowance[buyer][contract]>=bidding price)
    
    */
    //seller call exchange function to finish the transaction
    function change(Order calldata order,Sig calldata sig,address payable nftseller) payable external{
        ...
        //transferWithFeesPossibility(order.key.buyAsset, order.buying, msg.sender, order.key.owner, false, order.sellerFee,0, order.key.sellAsset);
    }
}